...
class taskA_Processor(DataProcessor):
    def get_train_examples(self, data_dir):
        ...
        return self._create_train_examples(
            self._read_tsv(os.path.join(data_dir, "OffensEval_train.tsv")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_dev_examples(
            self._read_tsv(os.path.join(data_dir, "OffensEval_dev.tsv")), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_test_examples(
            self._read_tsv(os.path.join(data_dir, "testset-levela.tsv")), "test")
...
def main():
    ...
    if args.do_test and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        test_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_guids)
        ...
        for input_ids, input_mask, segment_ids, guids in tqdm(test_dataloader, desc="Prediction"):
            input_ids = input_ids.to(device)
            input_mask = input_mask.to(device)
            segment_ids = segment_ids.to(device)
            ids += guids.tolist()
